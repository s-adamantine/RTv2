...
/*   Updated: 2018/04/12 18:36:55 by mpauw            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	handle_button_click(t_event *event, t_menu_p p)
{
	...
	while (i < menu->sub_m_count)
	{
		...
		if ((p.type == MAIN_BUTTON && (menu->sub_m[i]).type == SUB_MENU))
			(menu->sub_m[i]).showing = 0;
		i++;
	}
	if (p.type == TAB_BUTTON || p.type == MAIN_BUTTON)
	{
		set_parents_showing(menu, p.id);
		set_children_showing(menu, p.id);
	}
